...
#define         DEVICE_PORT             "/dev/ttyO4"        // ---BA ttyO4 is a capital O not zero!!!
...
void Func_TCPServerHandleAndSendData(int newsockfd);
...
void KeyboardMon()
{
	...
	{
		input=getchar();
		if(input == 't') // Compare input to 't' character
		{
...
void Func_TCPClientHandleAndSendData(int sockfd)
{
	...
	char	TCPClient_SendPayloadBuffer[128];
		...
		if(bTCPClient_SendControllerUpdatesToServer || bTCPClient_SendCheckForConnectionStatus || bTCPClient_SendDataReceivedACK || bTCPClient_SendTestDataToServer)
		{
			...
			TCPClient_OutBuffer[iTCPClient_SendPayloadLen + 6] = 0x9e;
		...
		if(bTCPClient_DataReadyToSend)
		{
			...
			iTCPServer_Ret = write(sockfd,TCPServer_OutBuffer,(iTCPClient_SendPayloadLen + 6));// Send the command on the TCPClient port
			if (iTCPClient_Ret!=1) {                                                           // If the writting operation failed ...
...
//					else if(iSerial_ReceiveBytesRead == -2)
//						printf ("Error while reading !\n");                   // If not, print a message.
//					else if(iSerial_ReceiveBytesRead == -1)
//						printf ("Error setting timeouts !\n");                   // If not, print a message.


				//else if(iSerial_ReceiveBytesRead == 0)
...
/*
public static uint16_t ComputeChecksum(uint8_t* bytes, uint16_t length)
{
       uint16_t crc = 0;
...
uint16_t Func_Serial_CheckCRC(uint16_t iFrameStart, uint16_t iPayloadLen)
{
	uint16_t iCount1, iCount2, iResult;

	iCRC = 0xffff;

	for(iCount1 = 0; iCount1 < (iPayloadLen + 2);iCount1++)
	{
		iCRC ^= SerialInBuffer[iCount1 + iSerial_FrameStartCount];

		for(iCount2 =0; iCount2< 8;iCount2++)
		{
			iResult = iCRC & 1;
			iCRC >>= 1;
	...
	if((SerialInBuffer[iSerial_FrameStartCount+iCount1] == (iCRC & 0xff)) && (SerialInBuffer[iSerial_FrameStartCount + iCount1 + 1] == (iCRC >> 8)))
		return(1);
	else
		return(0);
}
...
void Func_Serial_MakeCRC(uint16_t iSendPayloadLen)
{
	uint16_t iCount1, iCount2, iResult;

	iCRC = 0xffff;

	for(iCount1 = 0; iCount1 < (iSendPayloadLen + 2);iCount1++)
	{
		iCRC ^= SerialOutBuffer[iCount1];
	...
	printf("Got here - iCRC = %d\n", iCRC);

//	Serial_SendCRCBuffer[0] = iCRC & 0xff;
//	Serial_SendCRCBuffer[1] = iCRC >> 8;
...
// close(fd);
//
// IPAddrString = inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr);
//
// /* display result */
// printf("%s\n", IPAddrString);
//
	 ...
	 if(!bTCPServerPortError)
	 {
		 while(!bKillThread_BBBLTM_TCPServerComms)
		 {
			...
			if(bTCPServer_ReqToSendDataReceivedACK)
			{
				...
				Func_TCPServerHandleAndSendData(newsockfd);
			...
			else
			{
				...
				iTCPServer_ReceiveBytesRead = read(newsockfd,TCPServer_InBuffer,127);
				//iTCPServer_ReceiveBytesRead = recv(newsockfd,TCPServer_InBuffer,127,0);
				//if (iTCPServer_ReceiveBytesRead < 0) printf("\nERROR reading from socket due to : [%s]\n",(char*)strerror(errno));

				// The final character of the string must be a line feed ('\n')
				if (iTCPServer_ReceiveBytesRead>0)
				//((iTCPServer_ReceiveBytesRead = read(newsockfd,TCPServer_InBuffer,127))>0)
				{
					//Signal data received to start packet decode process
					bTCPServer_ReceiveDecodeComplete = false;
...
void Func_TCPServerHandleAndSendData(int newsockfd)
{
	...
	uint16_t	iTCPServer_SendPayloadLen = 0,
				iTCPServer_ThisControllerNum = 99,
		...
		if(TCPServer_SendDataReceivedACK)
		{
			...
			//Set the end of Frame Flag
			TCPServer_OutBuffer[iTCPServer_SendPayloadLen + 6] = 0x9e;
		...
		if(bTCPServer_DataReadyToSend)
		{
			...
			iTCPServer_Ret = write(newsockfd,TCPServer_OutBuffer,(iTCPServer_SendPayloadLen + 6));
					 if (iTCPServer_Ret < 0) printf("ERROR writing to socket. \n");


			                                       // Send the command on the serial port
			if (iTCPServer_Ret!=1) {                                                           // If the writting operation failed ...
				printf ("Error while writing data\n");                              // ... display a message ...
	                                 // Send failure...maybe due to packet collision so trigger a read operation next cycle
			}
			else
