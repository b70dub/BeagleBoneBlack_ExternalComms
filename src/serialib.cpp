 ...
 * Modified From: example by Philippe Lucidarme, modified to run on Beaglebone Black
...
char serialib::WriteString(const char *String)
{
    ...
    if(!WriteFile(hSerial,String,strlen(String),&dwBytesWritten,NULL))  // Write the string
    {
		...
		printf ("Got Here - Send Error : %d",dwError);
...
/*!
     ...
     \return -1 error while writting data
  */
int serialib::Write(const void *Buffer, const unsigned int NbBytes)
{
#if defined (_WIN32) || defined( _WIN64)
