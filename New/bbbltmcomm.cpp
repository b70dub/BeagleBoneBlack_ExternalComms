...
void KeyboardMon()
{
	...
	//Monitor for program exit request
	while(bSerialCommsRunning || bTCPServerCommsRunning || bTCPClientCommsRunning)
	{


		if(bKillThread_BBBLTM_SerialComms && bSerialCommsRunning)
			printf("Serial Comms still running\n");

		if(bKillThread_BBBLTM_TCPServerComms && bTCPServerCommsRunning)
			printf("bTCPServer Comms still running\n");

		if(bKillThread_BBBLTM_TCPClientComms && bTCPClientCommsRunning)
			printf("TCPClient Comms still running\n");
	 ...
	 if(!bTCPServerPortError)
	 {
		 while(!bKillThread_BBBLTM_TCPServerComms)
		 {
			...
			else
			{
				...
				//iTCPServer_ReceiveBytesRead = recv(TCPServer_newsockfd,TCPServer_InBuffer,127,0);
...
*/
			...
			iTCPServer_Ret = write(sockfd,TCPClient_OutBuffer,(iTCPClient_SendPayloadLen + 7));// Send the command on the TCPClient port

			//iTCPServer_Ret = send(sockfd,TCPClient_OutBuffer,(iTCPClient_SendPayloadLen + 7),0);// Send the command on the TCPClient port
